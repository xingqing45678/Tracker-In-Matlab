...
function H = fhog_features_less(I,M,O,binSize,nOrients,softBin,useHog,clip)
% INPUTS
...
%  H        - [w/binSize x h/binSize x nOrients] gradient histograms
    ...
    R2 = zeros(1,wb*hb*nOrients);H1=zeros(1,wb*hb*output_sz);H=zeros(hb,wb,output_sz);
    R1 = gradientHist(M,O,h,w,binSize,output_sz,softBin,1);%调用梯度直方图函数
...
%     R2 =R1;
	...
	H1=hogChannels(H1,nbo,0, R1, N, hb, wb, output_sz, clip, 1);
    for i=1:output_sz
        for j=1:wb
            for k=1:hb
