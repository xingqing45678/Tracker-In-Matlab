function [tracker] = initTracker_webcam(im, tracker, p)
	...
	%% INITIALIZATION
	tracker.pos = p.init_pos;%目标初始中心位置
    tracker.target_sz = p.target_sz;%目标框大小
    ...
    tracker.hann_window = single(hann(p.cf_response_size(1)) * hann(p.cf_response_size(2))');%加汉宁窗，并将结果变为单精度
