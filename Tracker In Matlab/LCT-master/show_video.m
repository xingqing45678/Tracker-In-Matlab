...
%         im=imresize(im,1/resize_image);
        ...
        if isempty(tex_handle),
            tex_handle = text(5, 18, strcat('#',num2str(frame)), 'Color','w', 'FontWeight','bold', 'FontSize',15,'Parent',axes_h);
        end
        
		if ~isempty(boxes{frame}),
			set(rect_h, 'Visible', 'on', 'Position', boxes{frame});
            set(tex_handle, 'Visible', 'on','String', strcat('#',num2str(frame)));
		else
			set(rect_h, 'Visible', 'off');
            set(tex_handle,'Visible', 'off');
		end
	end
