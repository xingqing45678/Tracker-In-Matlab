...
%                 end
            ...
            if p.visualization_dbg      %为1时画图，0不画图
...
%         rect_position_padded = [pos([2,1]) - bg_area([2,1])/2, bg_area([2,1])];
        ...
        positions(frame,:) = pos;  %to calculate precision
        %% VISUALIZATION
        if p.visualization %为0时不画图，1时画图
            if 0%~isToolboxAvailable('Computer Vision System Toolbox')
...
%                             pause(0.001);
                    ...
                    %保存输出图像
%                     print(1,'-r600','-djpeg',fullfile(output_path,num2str(frame,'%06d')));
            end
        end
    end
    elapsed_time = toc;
