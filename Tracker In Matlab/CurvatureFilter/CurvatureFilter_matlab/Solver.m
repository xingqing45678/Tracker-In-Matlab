...
[BC_row,BC_col]=meshgrid(2:2:m-1,2:2:n-1);[BT_row,BT_col]=meshgrid(3:2:m-1,3:2:n-1);%生成网络采样点
[WC_row,WC_col]=meshgrid(2:2:m-1,3:2:n-1);[WT_row,WT_col]=meshgrid(3:2:m-1,2:2:n-1);%_row时行复制,_col时列复制，两者都是m/2*n/2维矩阵
BC=sub2ind(size(im),reshape(BC_row,size(BC_row,1)*size(BC_row,2),1), reshape(BC_col,size(BC_col,1)*size(BC_col,2),1));%重新调整矩阵行列维度，将其变为一行
BT=sub2ind(size(im),reshape(BT_row,size(BT_row,1)*size(BT_row,2),1), reshape(BT_col,size(BT_col,1)*size(BT_col,2),1));%将下标转化成索引值，以列为计数方向
WC=sub2ind(size(im),reshape(WC_row,size(WC_row,1)*size(WC_row,2),1), reshape(WC_col,size(WC_col,1)*size(WC_col,2),1));%生成所有BC（黑圆）像素点的索引值，如514代表坐标（2,2）
WT=sub2ind(size(im),reshape(WT_row,size(WT_row,1)*size(WT_row,2),1), reshape(WT_col,size(WT_col,1)*size(WT_col,2),1));%以此类推声称所有BTWCWT像素点的索引值
%% the neighbors' index
BC_pre = BC -1;BC_nex = BC +1;BC_lef = BC -m;BC_rig = BC +m;BC_lu = BC-1-m;BC_ru = BC-1+m;BC_ld=BC+1-m;BC_rd=BC+m+1;%将所有BC像素点的8领域索引值表示
BT_pre = BT -1;BT_nex = BT +1;BT_lef = BT -m;BT_rig = BT +m;BT_lu = BT-1-m;BT_ru = BT-1+m;BT_ld=BT+1-m;BT_rd=BT+m+1;%pre上，net下，left左，rig右，lu左上ld左下ru右上rd右下
...
for i = 1:MaxItNum%迭代次数
    ...
    result = myfun(result,WT,WT_pre,WT_nex,WT_lef,WT_rig,WT_lu,WT_ld,WT_ru,WT_rd, im_orig, DataFitOrder, Lambda);%四种像素点独立并行地GC曲率滤波
end
Energy = Energy(1:i,:);

%% %%%%%%%%%%%%%%%%%%%% three projection operaters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function result = proj_TV(im,BT,BT_pre,BT_nex,BT_lef,BT_rig,BT_lu,BT_ld,BT_ru,BT_rd, im_orig, DataFitOrder, Lambda)%GC曲率滤波采用分解方法去除相邻像素的依赖
BT5 = 5*im(BT); dist = zeros(size(BT_pre,1),8,'single');
dist(:,1) = im(BT_pre) + im(BT_nex) + im(BT_lu) + im(BT_lef) + im(BT_ld) - BT5; %上+下-当，左+右-当，左上+右下—当，左下+右上-当
...
dist(:,5) = im(BT_pre) + im(BT_lef) + im(BT_lu) + im(BT_ru) + im(BT_ld) - BT5; %左上三角切平面像素相加-当前
dist(:,6) = im(BT_pre) + im(BT_rig) + im(BT_ru) + im(BT_lu) + im(BT_rd) - BT5;%以此类推左下，右上，右下
dist(:,7) = im(BT_nex) + im(BT_lef) + im(BT_ld) + im(BT_lu) + im(BT_rd) - BT5; 
dist(:,8) = im(BT_nex) + im(BT_rig) + im(BT_rd) + im(BT_ld) + im(BT_ru) - BT5;

dist = dist/5; %% minimal projection%最小距离投影，将之前想家的平均化
dist= dist'; tmp = abs(dist); [v,ind] = min(tmp);%找出最小投影算子数值，v为各列最小值，ind为每个最小值对应的行号
tmp = sub2ind(size(dist),ind',(1:size(dist,2))');%将每列最小算子坐标转化成索引值
d_min = dist(tmp);
