...
%     im = imread([params.path s_frames{frame}]);
    ...
    if frame > startframe && params.learning_rate > 0 || frame == startframe && ~params.update_projection_matrix
    ...
    if train_tracker     
        ...
        % Do training
        if frame == startframe && params.update_projection_matrix
...
%             text(30, 30, int2str(frame), 'Color','y', 'FontSize',15);
%             hold off;
...
%         end
        if frame==startframe
            [fig_handle] = run_imshow_qw(im_to_show,rect_position_vis,frame);
        else
            [fig_handle] = run_imshow_qw(im_to_show,rect_position_vis,frame,fig_handle,...
